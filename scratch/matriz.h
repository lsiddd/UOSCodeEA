...
public:
	...
	void setDimensions(unsigned long rows, unsigned long columns, const T& val);
private:
    vector<T> elements;
    Row<T> row {&elements};
...
template<typename T>
void matriz<T>::setDimensions(unsigned long rows, unsigned long columns, const T& val) {
	this->rows = rows;
	this->columns = columns;
	row.setColumns(columns);
	elements.assign(rows*columns, val);
}
