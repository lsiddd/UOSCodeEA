						...
						if (tp_num == 4) //schedule every 4 seconds. This is because we calculate the throughput in a windows of 5 positions to find average throughput of every user.
						{
							...
							{	
								...
								for (uint16_t i = 0; i < ueNodes.GetN() ; i++)
								{
									...
									if ( ( Window_avg_Delay[i] > Total_UE_Del_Avg * 1.2) || (Window_avg_Packetloss[i] > Total_UE_PL_Avg * 1.2)) // //|| Window_avg_Packetloss[i] >= Total_UE_PL_Avg )) // puede analizar poniendo que si esta por encima de 50% de perdida de paquetes lo coloco en la lista.
									{
